                  ...
                  {cart.map((item, index) => (
                    <div key={`${item.id}-${index}`} className='flex flex-col gap-4 rounded-[32px] border border-slate-200 bg-white p-5 md:flex-row md:items-center md:justify-between'>
                      ...
                      <div className='flex flex-col items-start gap-3 md:items-end'>
                        ...
                        <button onClick={() => removeFromCart(index)} className='rounded-full border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100'>Remove</button>
                      </div>
                    </div>
                  ))}
                </div>
